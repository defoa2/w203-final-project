...
Furthermore, the correlation matrix below shows a weak correlation (r=0.158) between our two independent variables: rate and length. We can therefore conclude that there is no perfect or near perfect multicollinearity in our model.

```{r}
# correlation plot for the continuous variables
ggpairs(youtube_data[c(3, 5:8)])
```

```{r}
youtube_data$log_views <- log(youtube_data$views)

log_model <- lm(log_views~rate+length, data=youtube_data) # model with log transformation on views
vif(log_model)
...
## Q1.5 Normal Distribution of Errors
