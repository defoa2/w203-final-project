...
# histogram of likert variables - doesn't work unless vars are numeric
...
plot(log(my_data$Student.Attendance + 0.01), my_data$Student.Performance.Reading.and.Math)
...
heatmap_plot <- heatmap_plot + labs(y = "Student.Performance.Reading.and.Math")
...
gvif_results <- vif(model_complex)
#select only the index and last column (GVIF^(1/(2*Df)))
gvif_sqrt_column <- gvif_results[, ncol(gvif_results)]
result_df <- data.frame(GVIF_sqrt = gvif_sqrt_column)
result_df
```
