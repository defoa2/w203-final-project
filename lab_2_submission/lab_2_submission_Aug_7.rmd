...
stargazer(model_naive, learning_environment, controls, model_complex,
          ...
          type = "text", # Output in LaTeX format
          digits=2,
          font.size="small"
          ) 
```

Across all models, the beta coefficient on $Student.Attendance$ was highly significant (p<0.01), indicating a strong positive relationship with student success. The coefficient decrease as more features are added to the model, ranging from 2.92 in the Naive model to 1.08 in the Complex model. This indicates that our model is initially overestimating the effect size of $Student.Attendance$.

To illustrate this concept, consider a hypothetical school with a Student Attendance rate of 90%. According to the Naive model, if the Student Attendance rate increases by 10 percentage points (to 100%), the student success metric is predicted to increase by approximately 29%. This is over double the increase compared to the complex model, which would increase $Student.Success$ by only ~11%.

Despite $Student.Attendance$ being significant, $Teacher.Attendance$ was not significant in any of the models with a 95% confidence interval. 

Two variables, $School.TrackTrack E$ and $Years.on.Probation$, appear to have a significant negative impact on student success in the Controls and Complex models. This indicates that schools on a specific track or with more years on probation have lower rates of student.success, which intuitively makes sense. 

Several variables, including $Suspensions.Percent$, $Suspension.Days$, $Longitude$, and $Latitude$, were included in the Complex model but did not show statistical significance. This is despite $Longitude$ having a strong postive relationship with our target variable, $Student.Success$. This could suggest that these factors are not strong predictors of student success, at least within the context of this complex model.

Ultimately, our Control model works best as it explains the same degree of variability as our Complex model (Adj R^2 = 52%), but with 10 less features. Because the complex model could be reducing the effect size of important predictors, like student attendance, we favor occums razor in this situation, and therefore conclude that our Control model performs the best. 
...
Through our analysis, we can conclude a student's success is significantly impacted by their school attendance, school status, and learning environment, including variables like $Involved.Families$, $Supportive.Environment$, and $Safety$. 

Since student success can benefit society in many ways, from increasing tax revenue to creating a more vibrant society[^5], it is crucial for superintendents to use this information to align priorities when developing future roadmaps to drive student success. As a starting point, superintendents can focus on increasing student attendance, which had the largest coefficient amongst our significant predictor variables. As shown in our best-performing model, the Control model, for every percent change in student attendance, student success increases 1.24%. Given these results, Chicago elementary school superintendents could prioritize reforms that help increase student attendance, particularly in southern and eastern parts of the city.
